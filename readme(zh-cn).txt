...
conda activate pytorch
cd YOUR_DIR_PATH
python run.py --pattern long_segment --model module_31.pth
#python run.py命令
...
glob遍历cnvkit_do\\fa下的所有.txt文件,写入cnvkit_do\connect文件夹下

############################################
###模型说明
1.ResNet_Attention.py	#带有注意力机制的残差卷积模型
来自本科毕业论文"eccDNA Identification based on Deep Learning".配合module_dict_38.pth使用。
2.ResAttention.py	#穿插注意力机制的网络模型
性能更佳，配合6.pth使用，需在run.py文件中修改模型调用代码
3.经过测试transformer模型并不适用于本任务，可能由于数据量过少或单个数据的信息密度过低
